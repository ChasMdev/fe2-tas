...
Animation.AnimationId = "rbxassetid://"+(ReplicatedStorage.WaitForChild("AnimIDs").WaitForChild("WallHang") as IntValue).Value
...
humRootPart.Touched.Connect(() => {
    ...
    if (notSwimming && (state === Enum.HumanoidStateType.Jumping || state === Enum.HumanoidStateType.Freefall) && !isWallHanging) {
        const [part, intersection, normal] = Workspace.FindPartOnRay(
            new Ray(
                humRootPart.Position,
                humRootPart.CFrame.LookVector.mul(2.5)
            ),
            char
        )
        if (part && part.FindFirstChild("_Wall") && lastWalljumpTime + 0.1 < os.clock()) {
            ...
            if (hasJumped) {
                humRootPart.Velocity = new Vector3(
                    normal.X * 52,
                    normal.Y * 52 + humanoid.JumpPower,
                    normal.Z * 52
                );
