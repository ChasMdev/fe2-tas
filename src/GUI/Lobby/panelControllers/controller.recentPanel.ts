...
export class recentPanelControl {
	...
	public moveRecentPanel(
		...
		if (visible) {
			views.CurrentView = ViewPorts.RECENT;
			task.wait(0.07);
			this.positionMotor.setGoal({
				posX: new Spring(newPosition.posX, springProps),
				posY: new Spring(newPosition.posY, springProps),
			});
			task.spawn(() => {
